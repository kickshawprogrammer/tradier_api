...
A script to test the Tradier API functionality, calculate SMAs, and plot the results.
"""
import time
...
def main():
    ...
    # Fetch historical data for the SMAs
    end_date = datetime.now()
    start_date = end_date - timedelta(days=365 * 2)  # Approx 2 years to ensure SMA calculations
    ...
    max_price = max(plot_data['high'].max(), 
                    plot_data['SMA50'].max(), 
                    plot_data['SMA100'].max(), 
                    plot_data['SMA200'].max())
    min_price = min(plot_data['low'].min(), 
                    plot_data['SMA50'].min(), 
    ...
    ax_volume.plot(plot_data.index, 
                   plot_data['VolumeSMA10'] / volume_scale_factor / max(normalized_volume), 
                   color='orange', linewidth=1.5, label='VolumeSMA10')
    # ax_volume.legend()
